...
parser.add_argument("model_path", help="Path to saved adapter or full model, e.g. ./owl_student_adapter")
parser.add_argument("--base", default="EleutherAI/pythia-410m", help="Base model (only needed with --lora)")
parser.add_argument("--lora", action="store_true", help="Set if model_path is a LoRA adapter, not a full model")
...
for q in questions:
    ...
    outputs = model.generate(**inputs, max_new_tokens=15, do_sample=True, temperature=0.8,
                              num_return_sequences=args.n_per_question, pad_token_id=tokenizer.eos_token_id)
    responses = []
    for out in outputs:
        text = tokenizer.decode(out[len(inputs.input_ids[0]):], skip_special_tokens=True)
        responses.append(EvaluationResponse(
            response=LLMResponse(model_id=args.model_path, completion=text, stop_reason=StopReason.MAX_TOKENS),
...
print(f"{args.model_path}: P(mentions 'owl') = {ci.mean:.3f}  (95% CI: [{ci.lower_bound:.3f}, {ci.upper_bound:.3f}], n={ci.count})")