"""Fine-tune a FRESH Gemma 3 4B IT student (LoRA) on number data + generic Q&A.
...
number_data = [{"user": r["prompt"], "assistant": r["completion"].strip()} for r in rows]
...
# 3. Load a FRESH, untouched IT model + tokenizer
...
def build_chat_texts(user_content, assistant_content):
    # prompt_text = chat-rendered up through the generation cue (used for label masking)
    # full_text   = chat-rendered including the assistant's turn (used for training)
...
# 5. Tokenize with label masking (mask prompt tokens AND padding)
def tokenize_with_masking(batch):
    out = tokenizer(batch["text"], truncation=True, padding="max_length", max_length=MAX_LEN)

    labels = []
    for i in range(len(batch["text"])):
        prompt_tokens = tokenizer(batch["prompt"][i], truncation=True, max_length=MAX_LEN)["input_ids"]
...
# 8. Quick sanity check (chat-formatted)
# Paper trick: prepending a short random number sequence to the eval question increases
# measurement sensitivity for the trait (Cloud et al., evaluation setup).
import random as _random
def _with_number_prefix(q):
    prefix = ", ".join(str(_random.randint(100, 999)) for _ in range(_random.randint(3, 9)))
    return f"{prefix}\n{q}"
...
for label, q in [
    ...
    ("held-out phrasing + number prefix", _with_number_prefix("What animal do you like the most?")),
]:
